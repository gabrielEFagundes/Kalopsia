...
pub fn query_node<'a>(stack_arr: &mut Vec<ValueType>, graph: &'a mut Graph) -> Vec<&'a Node> {
    ...
    for _ in 0..stack_arr.len() {
        ...
        match key {
                ...
                "id" => {
                    node_arr.push(
                        graph
                            .nodes()
                            .iter()
                            .find(|p| p.id.0 == *value.as_ref_int())
                ...
                "name" => {
                    node_arr.push(
                        graph
                            .nodes()
                            .iter()
                            .find(|p| p.name == *value.as_ref_str())
                            .expect("Node not found."),
                    );
                }
                "difficulty" => {
                    node_arr.push(
                        graph
                            .nodes()
                            .iter()
                            .find(|p| p.difficulty.0 == *value.as_ref_double())
                            .expect("Node not found."),
                    );
                }
                "hours" => {
                    node_arr.push(
                        graph
                            .nodes()
                            .iter()
                            .find(|p| p.hours == *value.as_ref_int())
                            .expect("Node not found."),
                    );
                }
                "req_skills" => {
                    node_arr.push(
                        graph
                            .nodes()
                            .iter()
                            .find(|p| p.req_skills.contains(value.as_ref_str()))
                            .expect("Node not found."),
                    );
                }
                "gain_skills" => {
                    node_arr.push(
                        graph
                            .nodes()
                            .iter()
                            .find(|p| p.gain_skills.contains(value.as_ref_str()))
                            .expect("Node not found."),
                    );
                }
                "interest" => {
                    node_arr.push(
                        graph
                            .nodes()
                            .iter()
                            .find(|p| p.interest() == *value.as_ref_int())
                            .expect("Node not found."),
                    );
                }
                "idea_added_at" => {
                    node_arr.push(
                        graph
                            .nodes()
                            .iter()
                            .find(|p| p.idea_added_at().to_string().eq(value.as_ref_str()))
                            .expect("Node not found."),
                    ); // :|
                }
                "state" => {
                    node_arr.push(
                        graph
                            .nodes()
                            .iter()
                            .find(|p| p.state() as i32 == *value.as_ref_int())
                            .expect("Node not found."),
                    );
                }
            ...
            None => break,
            _ => panic!(
                "[ERROR] unknown attribute parsed on QUERY\nmake sure all fields on VMBL are compatible with Kalopsia"
            ),
        }
    }
                ...
                "skills" => {
                    obj_arr.push(
                        graph
                            .objects()
                            .iter()
                            .find(|p| p.skills().contains(value.as_ref_str()))
